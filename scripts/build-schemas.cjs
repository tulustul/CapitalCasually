...
function generateSchema(type) {
  const schemaOutput = execSync(
    `typescript-json-schema tsconfig.app.json "${type}" --noExtraProps --required`,
  );
  const schema = JSON.parse(schemaOutput);

  const filepath = `public/data/schemas/${type.replace(
    "Json",
    "",
  )}.schema.json`;
...
const allTypes = ["DataIndex", "CapitalMetadata", "CapitalData"];
...
function getCliTypes() {
  ...
  for (const type of cliTypes) {
    if (!allTypes.includes(type)) {
      console.error(
        `Invalid type: ${type}. Must be one of: ${allTypes.join(", ")}`,
      );
      process.exit(1);
    }
  }
